...
outpath = '/home/rodur28/novaseq_run2_py/'
...
testing_sample = 'BY4741_10' # for now only 1 sample for testing --> entire group of samples sharing the same conditions are extracted
# For final run, need to update code to loop over the whole script for each entry in the df.groupby(lc) object
...
for x in gby[sample_label]:
    ...
    design_df = pd.DataFrame(columns=['sname','compound','preculture','experiment_replicate','selection_id','selection_replicate','technical_replicate','pair1','pair2','cutadapt5First','cutadapt5Second','generations','selection_time'])
    ...
    fulldesign_df['preculture'] = fulldesign_df.sname.apply(lambda x: rep_df.loc[rep_df[sample_label]==x, 'preculture'].values[0])
    ...
    fulldesign_df.to_csv('/home/rodur28/novaseq_run2_expdesign/'+cset_name+'_expdesign.txt', sep='\t', index=False) # Dataframe is saved with sample ids and compound just in case, for downstream analysis
    ...
    # Note - sample_name must contain alphanumeric characters only and typically won't work well if the name is complicated
